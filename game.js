(function () {
  ...
  var START_AMMO = 24;
  var MAX_AMMO = 99;
  var AMMO_DROP_CHANCE = 0.52;
  var AMMO_PICKUP = 12;
  var FIRE_COOLDOWN_MS = 180;
  var INVINCIBLE_MS = 1600;
  var HIT_COOLDOWN_MS = 900;
  var SPAWN_INTERVAL_MS = 2200;
  var ZOMBIE_BASE_HP = 3;
  var ZOMBIE_RUNNER_CHANCE = 0.16;
  var ZOMBIE_BRUTE_CHANCE = 0.10;
  ...
  var player, bullets, zombies, pickups, floatTexts;
  ...
  function init() {
    ...
    zombieImg.onerror = function () {
      // failed to load normal zombie sprite
    };
    zombieImg.src = "zombie sprite.png";

    bruteZombieImg = new Image();
    bruteZombieImg.onerror = function () {
      bruteZombieImg = null;
    };
    bruteZombieImg.src = "Big zombie.png";
  ...
  function spawnZombies(dt) {
    ...
    spawnTimer = 0;
    if (zombies.length >= Math.floor(6 + wave * 1.8)) return;
    ...
    var type = "normal";
    var hp = ZOMBIE_BASE_HP + Math.floor(wave / 3);
    var speed = 0.95 + wave * 0.08 + Math.random() * 0.35;
    var scoreValue = 10;
    var radius = 18;

    var roll = Math.random();
    if (roll < ZOMBIE_RUNNER_CHANCE) {
      type = "runner";
      hp = Math.max(1, ZOMBIE_BASE_HP - 1 + Math.floor(wave / 4));
      speed += 0.9 + Math.random() * 0.2;
      scoreValue = 14;
      radius = 16;
    } else if (roll < ZOMBIE_RUNNER_CHANCE + ZOMBIE_BRUTE_CHANCE) {
      type = "brute";
  ...
  function killZombie(z, index) {
    ...
    if (Math.random() < AMMO_DROP_CHANCE) {
      ...
      if (z.type === "brute" && Math.random() < 0.4) {
        pickups.push({
          x: z.x + 10,
          y: z.y - 10,
          r: 12,
  ...
  function drawZombie(z) {
    ...
    if (
      z.type === "brute" &&
      bruteZombieImg &&
      bruteZombieImg.complete &&
      bruteZombieImg.naturalWidth
    ) {
      ctx.drawImage(bruteZombieImg, z.x - z.r, z.y - z.r, size, size);
    } else if (zombieImg && zombieImg.complete && zombieImg.naturalWidth) {
      ctx.drawImage(zombieImg, z.x - z.r, z.y - z.r, size, size);
    } else {
      var fillColor = "#3dff9a";
      if (z.type === "runner") {
        fillColor = "#ffb348";
